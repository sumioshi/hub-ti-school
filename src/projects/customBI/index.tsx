...
import { LayoutGrid, Edit, RefreshCw, ArrowLeft } from 'lucide-react';
...
const UXCustomBIDemo: React.FC = () => {
  const [activeTab, setActiveTab] = React.useState<'customBI' | 'powerBI'>('customBI');
  const [selectedTemplate, setSelectedTemplate] = React.useState<string>('Varejo');
  const [selectedDataSource, setSelectedDataSource] = React.useState<string>('PowerBI');
  const [customDomain, setCustomDomain] = React.useState<string>('');
  const [realTimeData, setRealTimeData] = React.useState<string>('Nenhum dado recebido.');
  ...
  const [pbMode, setPbMode] = React.useState<'edit' | 'view'>('edit');
  const [pbLayout, setPbLayout] = React.useState<string>('Default');
  const [pbFilter, setPbFilter] = React.useState<string>('All');
  const [pbTheme, setPbTheme] = React.useState<string>('Dark');
  const [pbPermission, setPbPermission] = React.useState<boolean>(true);
  const [pbBookmark, setPbBookmark] = React.useState<string>('');
  const [pbBookmarks, setPbBookmarks] = React.useState<string[]>([]);
  const [pbDrillThrough, setPbDrillThrough] = React.useState<boolean>(false);
  ...
  const powerBIEventHandlers = new Map<string, (event?: any, embeddedEntity?: any) => void>();
